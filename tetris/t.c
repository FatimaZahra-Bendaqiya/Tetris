...
void drawBoard() {
    ...
    int y1cord = OFFSET,
        ...
        x1cord = (gfx_screenWidth() - blockwidth * COLUMNS) / 2,
        x2cord = gfx_screenWidth() - x1cord;
    gfx_line(x1cord - DISPLACEMENT, y1cord - DISPLACEMENT, x2cord + DISPLACEMENT, y1cord - DISPLACEMENT, BOARDCOLOR); //TOP
    gfx_line(x1cord - DISPLACEMENT, y2cord + DISPLACEMENT, x2cord + DISPLACEMENT, y2cord + DISPLACEMENT, BOARDCOLOR); //BOTTOM
    gfx_line(x1cord - DISPLACEMENT, y1cord - DISPLACEMENT, x1cord - DISPLACEMENT, y2cord + DISPLACEMENT, BOARDCOLOR); //LEFT
    gfx_line(x2cord + DISPLACEMENT, y1cord - DISPLACEMENT, x2cord + DISPLACEMENT, y2cord + DISPLACEMENT, BOARDCOLOR); //RIGHT
}
...
void pickNewPiece() {
    ...
    if (newpieceonscreen == false) {
        ...
        piece_color = BLUE;//randomInt(1, 7);
        if (piece_shape == 1)
            piece_variant = 3;
        else
            piece_variant = 0;
...
void drawBlocks(int array[ROWS][COLUMNS]) {
    ...
    const int OFFSET = 10,
              BLOCKWIDTH = (gfx_screenHeight() - 2 * OFFSET) / ROWS,
              ADDITIONALSPACE = 2;
    int x_base = (gfx_screenWidth() - BLOCKWIDTH * COLUMNS) / 2,
...
void checkHorizontalCollision() {
    ...
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLUMNS; j++) {
            if (pieceboard[i][j] && j >= 1 && board[i][j - 1]) {
    ...
    for (int i = 0; i < ROWS; i++) {
        for (int j = COLUMNS - 1; j >= 0; j--) {
            if (pieceboard[i][j] && j < COLUMNS - 1 && board[i][j + 1]) {
            ...
            else if (pieceboard[i][j] && j >= 0 && !board[i][j + 1])
                piece_collision_right = false;
        }
    }
}
